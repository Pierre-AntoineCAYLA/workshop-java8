...
public class Optional_02_Test {
    ...
    public void test_optional_map() throws Exception {
        ...
        Optional<Integer> julesAge = julesOpt.map(ju->ju.getAge());
    ...
    public void test_optional_ifPresent() throws Exception {
        ...
        julesOpt.filter(adult).map(ju->ju.getAge());
        ...
        // julesOpt.filter...;
        julesOpt.filter(adult).map(ju->ju.getAge()).ifPresent(ju-> {
        	GoodException exe =new GoodException();
        	throw exe;
        	});
        }
